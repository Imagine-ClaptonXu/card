...
const buildCardLib = function (debugData) {
	...
	if (debugData) {
		...
		// TODO: test 左上角牌叠用光的情况
		// TOP_CARD_LIB = TOP_CARD_LIB.slice(0, 1)
...
const buildEachColumnCard = function () {
	...
	for (let i = 0; i < eachColumn.length; i++) {
		...
		for (let i2 = 0; i2 < index; i2++) {
			...
			// 判断正反面
			let textColor = templateCardHtml(e2)
...
const handleChangeCardBtn = function () {
	$(".changeBtn").on("click", event => {
		...
		let textColor = templateCardHtml(topShowCard)
...
const handleChangeFace = function () {
	$(".isNegative").on("click", event => {
		...
		let html = templateCardHtml(data)
...
const updateTopCardList = function (nowDragDom) {
	...
	if ($(nowDragDom)[0]?.classList?.contains('subCard')) {
		...
		// 优化左上角牌叠被翻完的情况
		if (TOP_CARD_LIB.length <= 0) {
			$('.changeBtn')[0].innerHTML = '<div class="changeEmpty"></div>'
			$('.changeBtn').removeClass('changeBtn')
		}
		topCardIndex -= 1
	}
}
